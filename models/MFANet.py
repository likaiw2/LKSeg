...
def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    ...
    with torch.no_grad():
        ...
        # 截断
        tensor.data = torch.clamp(tensor.data, min=a * std, max=b * std)
    return tensor
...
class GlobalLocalAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 替代 rearrange 函数
        # 原始: q, k, v = rearrange(qkv, 'b (qkv h d) (hh ws1) (ww ws2) -> qkv (b hh ww) h (ws1 ws2) d', h=self.num_heads,
        ...
        # 重塑张量
        qkv = qkv.reshape(B, 3, h, d, hh, ws1, ww, ws2)
        qkv = qkv.permute(1, 0, 4, 6, 2, 5, 7, 3)  # 3, B, hh, ww, h, ws1, ws2, d
        qkv = qkv.reshape(3, B * hh * ww, h, ws1 * ws2, d)
        ...
        # 替代 rearrange 函数
        # 原始: attn = rearrange(attn, '(b hh ww) h (ws1 ws2) d -> b (h d) (hh ws1) (ww ws2)', h=self.num_heads,
        #                  d=C//self.num_heads, hh=Hp//self.ws, ww=Wp//self.ws, ws1=self.ws, ws2=self.ws)
        
        attn = attn.reshape(B, hh, ww, h, ws1 * ws2, d)
        attn = attn.permute(0, 3, 5, 1, 4, 2)  # B, h, d, hh, ws1*ws2, ww
        attn = attn.reshape(B, h * d, hh * ws1, ww * ws2)
...
class MFANet(nn.Module):
    def __init__(self,
        ...
        # self.sam = sam_model_registry["vit_b"](args,checkpoint='model_weights/sam_weight/sam_vit_b_01ec64.pth')
        self.sam = sam_model_registry["vit_l"](checkpoint='model_weights/sam_weight/sam_vit_l_0b3195.pth')
        # self.sam = sam_model_registry["vit_h"](args,checkpoint='model_weights/sam_weight/sam_vit_h_4b8939.pth')
        self.image_encoder = self.sam.image_encoder
        encoder_channels = (256, 256, 256, 256)
        
        self.superpixel_extractor = SuperpixelExtractor(superpixel_extractor)
    ...
    def forward(self, x):
        ...
        # 提取超像素掩码
        images_for_sp = x.clone()
        images_for_sp = torch.clamp(images_for_sp / 255.0, 0, 1)
        
        # 提取超像素掩码
        pred_masks_batch, n_pred_masks, covered_pixels_batch, assigned_masks_batch = self.superpixel_extractor(images_for_sp)
        
        # 将掩码转换为浮点类型并确保值范围合适，并移动到正确的设备上
        y = assigned_masks_batch.float().to(device)  # 转换为浮点类型并移动到设备上
        y = torch.unsqueeze(y, dim=1).repeat(1,3,1,1)
        
        # 继续原来的前向传播
        deepx = self.image_encoder(x)  # 256*16*16
        deepy = self.image_encoder(y)  # 256*16*16
