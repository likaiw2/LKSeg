...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, cost_class: float = 1, cost_mask: float = 1, cost_dice: float = 1, num_points: int = 12544):
        ...
        Params:
            ...
            num_points: number of points to sample for mask matching
        """
        super().__init__()
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        for b in range(bs):
            ...
            # Sample points for efficient matching
            point_coords = torch.rand(1, self.num_points, 2, device=out_mask.device)
            
            # Get gt labels at sampled points
            tgt_mask_sampled = F.grid_sample(
                tgt_mask.float(),
                point_coords.repeat(tgt_mask.shape[0], 1, 1).unsqueeze(2) * 2.0 - 1.0,
                align_corners=False,
                mode='nearest'
            ).squeeze(1).squeeze(2)

            out_mask_sampled = F.grid_sample(
                out_mask.float(),
                point_coords.repeat(out_mask.shape[0], 1, 1).unsqueeze(2) * 2.0 - 1.0,
                align_corners=False,
                mode='bilinear'
            ).squeeze(1).squeeze(2)
            ...
            with autocast(enabled=False):
                ...
                cost_mask = batch_sigmoid_ce_loss_jit(out_mask_sampled, tgt_mask_sampled)
