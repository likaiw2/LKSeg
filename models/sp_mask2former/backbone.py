...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            # Convert back to spatial format for output
            if f"res{i+2}" in self.out_features:
                B, L, C = x.shape
                H = W = int(math.sqrt(L))
                spatial_x = x.transpose(1, 2).view(B, C, H, W)
                features[f"res{i+2}"] = spatial_x
